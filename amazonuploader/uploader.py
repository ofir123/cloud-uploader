...
def upload_file(file_path):
    ...
    if video_type == 'episode' and title:
        ...
        if season:
            ...
            if episode:
                cloud_path = '{}/{}/Season {:02d}/{} - S{:02d}E{:02d}'.format(
                    config.TV_PATH, title, season, title, season, episode)
    elif video_type == 'movie' and title:
        year = guess_results.get('year')
        if year:
            cloud_path = '{}/{} ({})/{} ({})'.format(config.MOVIE_PATH, title, year, title, year)
    if cloud_path:
        if language_extension:
            cloud_path += language_extension
        cloud_path += file_extension
        logger.info('Cloud path: {}'.format(cloud_path))
        # Upload!
        process = subprocess.run([config.ACD_CLI_PATH, 'upload', file_path, cloud_path], shell=True,
